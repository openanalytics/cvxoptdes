#' Penalized generalized additive model optimal design
#'
#' @description
#' An R6 class for managing penalized locally optimal experiment designs for generalized additive models.
#' This class encapsulates the generalized additive model at a set of nominal parameter values, the discrete design grid, 
...
#' \item \eqn{M(w | X, v, \theta, F, g)} is the information matrix depending only on the weights \eqn{w} and \eqn{v}, and the design matrix \eqn{X} by default.
#' If a non-trivial distribution family \eqn{F} is chosen, the information matrix also depends on the nominal parameter values \eqn{\theta},
#' the chosen (exponential) distribution family \eqn{F} and the link function \eqn{g}.
#' \item \eqn{\Phi} is the chosen optimality criterion, generally a function of the information matrix.
#' }
#' By default, the error distribution family \eqn{F} is Gaussian with identity link function \eqn{g}. This implies that the information matrix
#' (conditional on the error variance) is independent of the model parameters and given by:
#' \deqn{M(w | X, v) \ \propto \ \sum_{i = 1}^n w_i v_i x_i x_i^T}
#' If \eqn{F} is not Gaussian or Gaussian with a non-canonical link function \eqn{g}, the information matrix is given by:
#' \deqn{
#' M(w | X, v, \theta, F, g) \ = \ \sum_{i = 1}^n w_i v_i u(x_i, \theta) x_i x_i^T
#' }
#' with weights \eqn{u(x_i, \theta) = \frac{1}{V(\mu_i)[g'(\mu_i)]^2}} depending on the parameter values \eqn{\theta}. Here, \eqn{\mu = E[Y] = b'(\zeta)}
#' and \eqn{V(\mu) = b''(\zeta)}, with \eqn{b(\zeta)} is the cumulant function of the exponential family \eqn{F}. See \link{glm_design} for the exact details.
#'
...
#'
#' @field formula a GAM model defined as a one-sided \link{formula}.
...
#' Only used if \code{$family} is not \code{gaussian(link = "identity")} (the default).
...
#' @field X the design matrix obtained via \code{mgcv::gam(formula, data = data, fit = FALSE, ...)} given the design points in \code{data} and the model \code{formula}.
...
#' gam_fit <- mgcv::gam(NOx ~ s(E) + C, data = ethanol[exact_design > 0, ])
#' summary(gam_fit)
#' 
...
#'   data = data
#' )
#' ## integer I-optimal design w/ 30 design points
#' exact_design <- design$round(m = 30, method = "optimal", criterion = "I", 
#'                              n_repeats = 25, seed = 1, design_weights = FALSE)
#' design$crit(exact_design, criterion = "I")
#' ## GAM fitted to integer design
#' gam_fit <- mgcv::gam(y ~ s(x1, x2), data = data[exact_design > 0, ])
#' summary(gam_fit)
#'
#' @export
gam_design <- R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Constructor. Initializes the experimental design to optimize with a linear model formula, discrete design grid, and optionally
    #' a distribution family with link function (and nominal set of parameter values).
    #' @param formula a GAM model defined as a one-sided \link{formula}, see \code{\link[mgcv]{gam}}.
    ...
    #' Only needed if \code{$family} is not \code{NULL}.
    ...
    #' @param ... any additional arguments passed to \code{mgcv::gam(formula, data = data, fit = FALSE, ...)} to evaluate the design matrix.
    initialize = function(formula, data, theta = NULL, family = NULL, weights = NULL, cost = NULL, contrasts = NULL, alpha = 1, lambda = 0, upper = 1, ...) {

      stopifnot("'family' must be an object of class \"family\", (e.g. gaussian(link = \"identity\")" = is.null(family) || inherits(family, "family"))
      if(!is.null(family)) {
      ...
      if(!is.null(family)) {
        stopifnot("if a non-trivial 'family' is specified, 'theta' must also be provided" = !is.null(theta))
      }
      ...
      ## add dummy response for mgcv::gam
      gam_frm <- update(self$formula, y ~ .)
      gam_G <- do.call(mgcv::gam, args = c(list(formula = gam_frm, data = cbind(self$data, y = 1), fit = FALSE), private$dots))
      gam_X <- gam_G$X
      colnames(gam_X) <- gam_G$term.names
      private$.Xinit <- gam_X
      if(!is.null(self$family)) {
        private$.X <- Xv_impl(private$.Xinit, theta = self$theta, family = self$family)
      } else {
        private$.X <- private$.Xinit
      }
      if(!is.null(weights)) {
    ...
    #' \code{alpha}, \code{lambda} and/or \code{upper} respectively.
    ...
    update = function(...) {
      ...
      if(is.element("family", names(dots))) {
        stopifnot("'family' must be an object of class \"family\", (e.g. gaussian(link = \"identity\")" = is.null(dots$family) || inherits(dots$family, "family"))
        if(!is.null(dots$family)) {
          if(identical(dots$family$family, "gaussian") && identical(dots$family$link, "identity")) {
            dots$family <- NULL
          }
        }
        if(!is.null(dots$family)) {
          stopifnot("if a non-trivial 'family' is specified, 'theta' must also be provided" = !is.null(dots$theta) || !is.null(self$theta))
        }
      ...
      if(is.element("theta", names(dots))) {
        if(!is.null(self$family)) {
          stopifnot("if a non-trivial 'family' is specified, 'theta' must also be provided" = !is.null(dots$theta) || !is.null(self$theta))
    ...
    #' @details The sparseness cost penalties are scaled by the same \code{alpha} tuning parameter as for a user-specific cost vector, see \code{\link{scs_control_dflt}}.
   ...
   #' Calculates the variance-covariance matrix of the model parameters based on an approximate or rounded (exact) design \eqn{(w_1, \ldots, w_n)} according to:
    ...
    sep = function(m, newdata, design_weights = NULL, type = c("link", "response"), sigma2 = 1) {
      ...
      if(!missing(newdata)) {
        ...
        ## add dummy response for mgcv::gam
        gam_frm <- update(self$formula, y ~ .)
        gam_G <- do.call(mgcv::gam, args = c(list(formula = gam_frm, data = cbind(newdata, y = 1), fit = FALSE), private$dots))
        gam_X <- gam_G$X
        colnames(gam_X) <- gam_G$term.names
        xpred <- gam_X
      } else {
      ...
      if(type == "response" && !is.null(self$family)) {
