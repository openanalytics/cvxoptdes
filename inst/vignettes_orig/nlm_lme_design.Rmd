...
opts_chunk$set(collapse = TRUE, error = TRUE, warning = FALSE, message = FALSE, eval = TRUE, comment = "#>",
...
```{r nlm_design-1, echo = FALSE, fig.dim = c(8, 6), out.width = "100%", fig.cap="Approximate locally I-optimal design"}
theta <- list(b1 = 1054.541, b2 = 206.546, b3 = 1.46, b4 = -0.26, b5 = 0.023, b6 = 0.401, b7 = 0.035, b8 = 57.405, b9 = -0.477)
xy <- expand.grid(
		x1 = seq(from = 0, to = 100, by = 10),
		x2 = seq(from = 0, to = 7.5, length = 50)
)
fx <- function(x1, x2, theta) {
  with(theta, b1/(b2 + x1) + b3 * x2 + b4 * x2^2 + b5 * x2^3 + (b6 * x2 + b7 * x2^3) * exp(-x1/(b8 + b9 * x2^2)))
}
xy <- transform(xy, f = fx(x1, x2, theta))

weights <- cbind(data, w = design$design_weights)
weights <- subset(weights, w > 1e-4)
weights <- transform(weights, f = fx(x1, x2, theta))

ggplot(data = xy, aes(x = x2, color = x1, group = x1)) +
		geom_line(aes(y = f)) +
    geom_point(data = weights, aes(y = f, fill = x1, size = w), color = "grey20", shape = 21) +
		scale_color_fermenter(palette = "RdBu", n.breaks = 8, limits = c(0, 100)) +
		scale_fill_fermenter(palette = "RdBu", n.breaks = 8, limits = c(0, 100)) +
		scale_size_area(n.breaks = 6, max_size = 7) +
		theme_light() +
		labs(x = "x2 (Pressure, atm)", y = "y (Kinematic viscosity)", color = "x1 (Temp., C)",
		     fill = "x1 (Temp., C)", size = "weight") +
...
```{r nlm_design-2, echo = FALSE, fig.dim = c(8, 6), out.width = "100%", fig.cap="Rounded (integer) design with m=11 design points"}
weights <- subset(data, exact_design > 0)
weights <- transform(weights, f = fx(x1, x2, theta))

ggplot(data = xy, aes(x = x2, color = x1, group = x1)) +
		geom_line(aes(y = f)) +
    geom_point(data = weights, aes(y = f, fill = x1), color = "grey20", shape = 21, size = 5) +
		scale_color_fermenter(palette = "RdBu", n.breaks = 8, limits = c(0, 100)) +
  	scale_fill_fermenter(palette = "RdBu", n.breaks = 8, limits = c(0, 100)) +
		theme_light() +
		labs(x = "x2 (Pressure, atm)", y = "y (Kinematic viscosity)", color = "x1 (Temp., C)", fill = "x1 (Temp., C)") +
    theme_minimal(base_size = 14)
```
...
```{r nlm_design-3, echo = FALSE, fig.dim = c(8, 6), out.width = "100%", fig.cap="Penalized approximate locally I-optimal design"}
weights <- cbind(data, w = design$design_weights)
weights <- subset(weights, w > 1e-4)
weights <- transform(weights, f = fx(x1, x2, theta))
...
```{r nlm_design-4, echo = FALSE, fig.dim = c(8, 6), out.width = "100%", fig.cap="Rounded penalized (integer) design with m=14 design points"}
weights <- design$subset(exact_design)
weights <- transform(weights, f = fx(x1, x2, theta))
...
```{r nlm_design-5, echo = FALSE, fig.dim = c(8, 6), out.width = "100%", fig.cap="Stratified approximate locally I-optimal design"}
xy1 <- subset(xy, x1 %in% c(0, 20, 40, 100))

weights <- cbind(design$data, w = design$design_weights)
weights <- subset(weights, w > 1e-4)
weights <- transform(weights, f = fx(x1, x2, theta))

ggplot(data = xy1, aes(x = x2, color = x1, group = x1)) +
		geom_line(aes(y = f)) +
    geom_point(data = weights, aes(y = f, fill = x1, size = w), color = "grey20", shape = 21) +
		scale_color_fermenter(palette = "RdBu", n.breaks = 8, limits = c(0, 100)) +
		scale_fill_fermenter(palette = "RdBu", n.breaks = 8, limits = c(0, 100)) +
...
```{r nlm_design-6, echo = FALSE, fig.dim = c(8, 6), out.width = "100%", fig.cap="Rounded stratified (integer) design with m=12 design points"}
weights <- design$subset(exact_design)
weights <- transform(weights, f = fx(x1, x2, theta))
...
The two-part model `formula` must always be of the form `~ fix.eff ~ ran.eff`, where `fix.eff` is a linear model formula encoding the fixed effects design matrix and `ran.eff` is an expression encoding the random effects levels. `ran.eff` is evaluated as a *factor*, with the factor levels determining the random effects groups. Using [`lme4`](https://CRAN.R-project.org/package=lme4)'s formula syntax, the two-part formula would be equivalent to `~(1 | Block) + Treatment + I(Treatment^2)`.
...
```{r, echo = FALSE}
exact_design <- numeric(nrow(design$data))
exact_design[c(9, 57, 81, 97, 127, 136, 184, 196, 226, 254, 296, 320, 345, 366, 399, 406, 463, 484, 489, 513, 549)] <- 1
```
...
```{r, lme_design-1, echo = FALSE, fig.dim = c(8, 6), out.width = "100%", fig.cap="Split-plot design fraction of design space plot"}
newdata <- expand.grid(
  A = seq(from = -1, to = 1, length = 10),
  B = seq(from = -1, to = 1, length = 10),
  C = seq(from = -1, to = 1, length = 10),
  D = seq(from = -1, to = 1, length = 10)
)

ses_exact <- design$sep(m = 21, newdata = newdata, design_weights = exact_design)
ses_doe1 <- design$sep(m = 21, newdata = newdata, design_weights = doe1_design)

plotdata <- data.frame(
  type = factor(rep(c("D-optimal", "DoE1"), each = nrow(newdata)), levels = c("D-optimal", "DoE1")),
  ses = c(sort(ses_exact^2), sort(ses_doe1^2)),
  frac = rep((1:nrow(newdata)) / nrow(newdata), times = 2)
)

ggplot(plotdata, aes(x = frac, y = ses, color = type, group = type)) +
    geom_line() +
    scale_color_manual(values = c("D-optimal" = "#386cb0", "DoE1" = "#9E0041")) +
    labs(x = "Fraction of design space", y = "Prediction variance", color = "") +
  theme_minimal(base_size = 14)
```

In the case study in [@OM25] there is one additional week (week 2) containing a public holiday during which only two runs are feasible. Instead of a single common whole plot size $m$, `$round_split_plot()` also accepts a vector $m$ of individual whole plot sizes. If $m$ is a vector, it must be the same length as the number of levels in the hard-to-change factor `A`:

```{r, echo = TRUE}
## append data w/ 1 extra week
data_8weeks <- transform(design$data, week = factor(week, levels = 1:8))
data_8weeks <- rbind(data_8weeks, transform(subset(design$data, week == 1), week = 8))

design_8weeks <- design$clone()
design_8weeks$update(data = data_8weeks)
```

```{r, eval = FALSE, echo = TRUE}
design_8weeks$optimize(criterion = "D", max_iter = 5, return_value = FALSE) 

mirai::daemons(n = parallel::detectCores() / 2, seed = 1)

exact_8weeks <- design_8weeks$round_split_plot(
  m = c(3, 2, 3, 3, 3, 3, 3, 3),          ## non-equal whole plot sizes                  
  hard_to_change = ~A,
  n_repeats = 1000
)
...
```{r, echo = FALSE}
exact_8weeks <- numeric(nrow(design_8weeks$data))
exact_8weeks[c(21, 60, 75, 82, 151, 214, 223, 235, 252, 294, 300, 331, 343, 379, 408, 432, 477, 500, 521, 566, 592, 604, 643)] <- 1
```

```{r, echo = TRUE}
design_8weeks$subset(replicates = exact_8weeks)
```
...
```{r, echo = FALSE}
penalized_design <- numeric(nrow(design$data))
penalized_design[c(1, 16, 73, 102, 138, 162, 184, 199, 223, 250, 295, 298, 349, 358, 391, 424, 436, 484, 494, 548, 560)] <- 1
```

```{r, echo = TRUE}
design$subset(replicates = penalized_design)
